...
def main() -> None:
    ...
    for tp, ep, dp_mode, global_bsz, fsep_mode in CONFIGS:
        ...
        try:
            ...
            # KeyError = missing per-(tp, ep, bsz) profile;
            # ValueError = config-shape constraint (dp*ep > micro_bsz, etc.)
            # In both cases the row is unsupported by the cost model under
            # the current calibration set; skip and continue so the sweep
            # produces a partial JSON instead of crashing.
            print(f"# skip tp={tp} ep={ep} {dp_mode} bsz={global_bsz} fsep={fsep_mode} ({type(e).__name__}: {e})")
            continue
        bd = est.breakdown
